<?php
	...
	if ($post == null) {
		// 로그인 페이지를 통해 로그인
		if (empty($_POST['id']) || empty($_POST['category']) || empty($_POST['team_name']) || empty($_POST['recruit_num']) || empty($_POST['time']) || empty($_POST['area']) || empty($_POST['date'])) {
		...
		$team_name = mysqli_real_escape_string($dbc, trim($_POST['team_name']));
	...
	// insert -----------------------------------------------------------------
	$query = "insert into match_info values " .
			"(null, $id, '$category', '$team_name', '$recruit_num', '$time', '$area', '$date', NOW())";
	//echo $query; 
